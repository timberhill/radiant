...
def savePeriodogramPlot(freqs, periods, powers, P, fit_p, subfolder=''):
	...
	plt.plot((P, P), (0, max(powers)), label='original period, P = ' + str(truncate(P)), color="#2c7873", linestyle="--")
	if fit_p > 0:
		plt.plot((fit_p, fit_p), (0, max(powers)), label='fitted period, P = ' + str(truncate(fit_p)), color="#fb6542", linestyle="-")
...
def saveRVPlot(ts, ts_full, rvs, rv_full_fit, rv_full_orig, rv_fit, rv_orig, Ms, SNR, params, params_fit, params_err, subfolder=''):
	...
	ax0.plot(ts_full, rv_full_orig, label='original curve', color="#2c7873", linestyle="--")
	ax0.plot(ts, rvs, label='datapoints', alpha=0.7, marker="o", markerfacecolor="#021c1e", markeredgecolor="none", linestyle="none")
	...
	# residuals
	ax1.plot((tmin, tmax), (0, 0), label='original curve', color="#2c7873", linestyle="--")
	ax1.plot(ts_full, rv_full_fit - rv_full_orig, label='fit', color="#fb6542")
	ax1.plot(ts, rvs - rv_orig, label='data points', alpha=0.7, marker="o", markerfacecolor="#021c1e", markeredgecolor="none", linestyle="none")

	ax2.plot((tmin, tmax), (0, 0), label='fit', color="#fb6542")
	ax2.plot(ts, rvs - rv_fit, label='data points', alpha=0.7, marker="o", markerfacecolor="#021c1e", markeredgecolor="none", linestyle="none")
